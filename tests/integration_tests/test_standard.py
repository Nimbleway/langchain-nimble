...
class TestNimbleSearchRetrieverStandard(RetrieversIntegrationTests):
    ...
    @pytest.mark.xfail(
        reason=(
            "Nimble API returns UP TO num_results, not exactly "
            "that number. We override to check <= instead of =="
        ),
        strict=False,
    )
    def test_k_constructor_param(self) -> None:
        """Test num_results constructor parameter."""
        params = {
            k: v
    ...
    def test_invoke_with_k_kwarg(self, retriever: BaseRetriever) -> None:
        """Test num_results in invoke()."""
        result_1 = retriever.invoke(
            self.retriever_query_example,
            None,
            **{self.num_results_arg_name: 1},
        )
        assert len(result_1) >= 1
        assert len(result_1) <= 1
        assert all(isinstance(doc, type(result_1[0])) for doc in result_1)
...
class TestNimbleExtractRetrieverStandard(RetrieversIntegrationTests):
    ...
    @pytest.mark.xfail(reason="Extract retriever doesn't support num_results")
    def test_k_constructor_param(self) -> None:
